...
var https = require('https');			// https 网路
	...
	var req = https.get(url, function(res) {
		...
		res.on('end', function(res) {
			...
			for(var i = 0; i < html.length; i++) {
                src = "https:" + html[i].attribs.src
				// 筛选部分广告，不是真的段子
				if (src.indexOf("https://image.haha.mx") > -1) {
					urls.push(src)
				}
			}
			// 递归调用
...
function downImg(imgurl) {
    console.log('imageUrl000:', imgurl);
	var narr = imgurl.replace("https://image.haha.mx/", "").split("/")
	// 做一步优化，如果存在文件，则不下载
